...
OWNERSHIP_LABEL_COUNT="$(printf '%s\n' "$CONTENT" | grep -c 'com.ferritelabs.tester.wrapper-guard: tester.sh')"
assert_eq 3 "$OWNERSHIP_LABEL_COUNT" "tester service, volume, and default network use the exact ownership label"
...
assert_contains "$CONTENT" "networks:" "tester Compose declares the default network explicitly"
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  RENDERED_OWNERSHIP_LABEL_COUNT="$(printf '%s\n' "$RENDERED" | grep -c 'com.ferritelabs.tester.wrapper-guard: tester.sh')"
  assert_eq 3 "$RENDERED_OWNERSHIP_LABEL_COUNT" "render labels the service, volume, and default network"
