...
# guard: automated dispatches must never regress the canonical
# active-release.env, equal retries are a no-op unless the checksum changed
# (which fails loudly), and only a manual workflow_dispatch may allow a
# downgrade. No network or Docker is required.
set -uo pipefail
...
# --- Static checks ----------------------------------------------------------
assert_contains "$CONTENT" "allow_downgrade:" \
  "version-sync.yml exposes a manual allow_downgrade input"
assert_contains "$CONTENT" "ALLOW_DOWNGRADE: \${{ inputs.allow_downgrade || false }}" \
  "allow_downgrade is read only from workflow_dispatch inputs, defaulting to false"
assert_not_contains "$CONTENT" "client_payload.allow_downgrade" \
  "a repository_dispatch client payload can never supply allow_downgrade"
...
run_guard() {
  local candidate_ver="$1" candidate_sha="$2" allow="$3" out="$4"
  : > "$out"
  (
    cd "$FIXTURE" &&
...
if run_guard "0.4.1" "$ONES" "false" "${TMP}/newer.out"; then
...
if run_guard "0.4.0" "$ZERO" "false" "${TMP}/equal_same.out"; then
...
if run_guard "0.4.0" "$ONES" "false" "${TMP}/equal_diff.out"; then
...
# Older candidate, no override: skip the downgrade.
if run_guard "0.3.9" "$ONES" "false" "${TMP}/older.out"; then
  assert_contains "$(cat "${TMP}/older.out")" "proceed=false" \
    "an older automated candidate skips the downgrade"
  assert_contains "$(cat "${TMP}/older.out.log")" "skipping downgrade" \
    "an older automated candidate reports the skipped downgrade"
else
  harness_fail "older candidate without override unexpectedly errored: $(cat "${TMP}/older.out.log")"
fi

# Older candidate WITH manual override: proceed.
if run_guard "0.3.9" "$ONES" "true" "${TMP}/override.out"; then
  assert_contains "$(cat "${TMP}/override.out")" "proceed=true" \
    "a manual allow_downgrade override permits the downgrade"
  assert_contains "$(cat "${TMP}/override.out.log")" "allow_downgrade was explicitly requested" \
    "the manual override is logged explicitly"
else
  harness_fail "older candidate with override unexpectedly errored: $(cat "${TMP}/override.out.log")"
fi

# Malicious candidate version: rejected by the shared guard, never executed.
MARKER="$(mktemp -u)"
if run_guard "0.4.9\$(touch ${MARKER})" "$ONES" "false" "${TMP}/malicious.out"; then
