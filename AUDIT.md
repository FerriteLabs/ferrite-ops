...
- `tests/run.sh` (fake-binary smoke test + Dockerfile/Helm static checks)
- `shellcheck` on every touched shell script
- `helm lint` / `helm template` on both charts (tool available in this environment)
- `docker build -t ferrite:test .` (Docker daemon available in this environment) as a real build check
